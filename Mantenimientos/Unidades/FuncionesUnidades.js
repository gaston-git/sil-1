$(document).ready(function () {
    ...
    $("#btnNuevo").click(function () {
        window.location.href = '/Mantenimientos/Unidades/Agregar.cshtml';
        return false;
    });

    $("#btnEditar").click(function () {
        if (idRegistro > 0)
            window.location.href = '/Mantenimientos/Unidades/Editar.cshtml?id=' + idRegistro;
        else
    ...
    $("#btnEliminar").click(function () {
        $.confirm({
            title: 'Confirmación',
            content: '¿Está seguro(a) de borrar la Unidad?',
            buttons: {
                Aceptar: function () {
                    if (idRegistro > 0)
                        window.location.href = '/Mantenimientos/Unidades/eliminar.cshtml?id=' + idRegistro;
                    else
